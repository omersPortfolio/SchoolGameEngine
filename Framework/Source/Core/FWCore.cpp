...
bool FWCore::IsKeyTyped(int value)
{
    assert(value >= 0 && value < 256);
    return m_KeyTyped[value];
}
...
LRESULT CALLBACK FWCore::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEWHEEL:
        {
            ...
            pFWCore->m_MouseWheel += (float)GET_WHEEL_DELTA_WPARAM(wParam) / (float)WHEEL_DELTA;

            if (GET_WHEEL_DELTA_WPARAM(wParam) > 0)
            {
                pEvent = new InputEvent(
                    InputEvent::DeviceType::Mouse,
                    InputEvent::DeviceState::ScrollUp,
                    (unsigned int)wParam);
            }
            if (GET_WHEEL_DELTA_WPARAM(wParam) < 0)
            {
                pEvent = new InputEvent(
                    InputEvent::DeviceType::Mouse,
                    InputEvent::DeviceState::ScrollDown,
                    (unsigned int)wParam);
            }
    ...
    case WM_CHAR:
        {
            pFWCore->m_KeyTyped[wParam] = true;

            // Send a input event to the event manager.
            InputEvent* pEvent = new InputEvent(
                InputEvent::DeviceType::Keyboard,
                InputEvent::DeviceState::Typed,
                (unsigned int)wParam);
