...
void ImGuiManager::StartFrame(float deltaTime)
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    io.MouseWheel = m_ScrollInput;// m_pFramework->GetMouseWheel();
...
void ImGuiManager::OnEvent(Event* pEvent)
{
    ...
    if (pEvent->GetType() == InputEvent::GetStaticEventType())
    {
        ...
        {
            if (pInputEvent->GetDeviceState() == InputEvent::DeviceState::Typed)
            {
                if (pInputEvent->GetKeyCode() > 0 && pInputEvent->GetKeyCode() < 0x100000)
                {
                    io.AddInputCharacter(pInputEvent->GetKeyCode());
                }
            }
            else if(pInputEvent->GetDeviceState() == InputEvent::DeviceState::Pressed)
            {
                io.KeysDown[pInputEvent->GetKeyCode()] = true;
            }
            else
            {
                io.KeysDown[pInputEvent->GetKeyCode()] = false;
            }
        }
        else if (pInputEvent->GetDeviceType() == InputEvent::DeviceType::Mouse)
        {
            if (pInputEvent->GetDeviceState() == InputEvent::DeviceState::ScrollUp)
            {
                m_ScrollInput++;
            }
            else if (pInputEvent->GetDeviceState() == InputEvent::DeviceState::ScrollDown)
            {
                m_ScrollInput--;
            }
        }
